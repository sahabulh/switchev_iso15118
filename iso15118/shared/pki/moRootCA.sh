...
# Create a self-signed moRootCA certificate
...
then
    ...
    openssl genpkey -algorithm $EC_CURVE $SYMMETRIC_CIPHER -pass pass:$password -out $KEY_PATH/moRootCA.key
fi
...
openssl x509 -req -in $CSR_PATH/moRootCA.csr -extfile configs/moRootCACert.cnf -extensions ext -signkey $KEY_PATH/moRootCA.key -passin pass:$password -set_serial 12353 -out $CERT_PATH/moRootCACert.pem -days $VALIDITY_MO_ROOT_CERT
