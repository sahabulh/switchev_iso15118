...
# Editor: Md Sahabul Hossain (sahabulh@iut-dhaka.edu)
# ===============================================================================================================
...
usage() {
  ...
  Options:
   ...
   -d --digicert      Use Digicert ONE for certificate management. Currently only MO is suppoerted.
                      Default: true
...
openssl req -new -key $KEY_PATH/v2gRootCA.key -passin pass:$password -config configs/$ISO_FOLDER/v2gRootCACert.cnf -out $CSR_PATH/v2gRootCA.csr
...
openssl x509 -req -in $CSR_PATH/v2gRootCA.csr -extfile configs/$ISO_FOLDER/v2gRootCACert.cnf -extensions ext -signkey $KEY_PATH/v2gRootCA.key -passin pass:$password $SHA -set_serial 12345 -out $CERT_PATH/v2gRootCACert.pem -days $VALIDITY_V2G_ROOT_CERT
...
openssl req -new -key $KEY_PATH/cpoSubCA1.key -passin pass:$password -config configs/$ISO_FOLDER/cpoSubCA1Cert.cnf -out $CSR_PATH/cpoSubCA1.csr
...
#      private key match.
openssl x509 -req -in $CSR_PATH/cpoSubCA1.csr -extfile configs/$ISO_FOLDER/cpoSubCA1Cert.cnf -extensions ext -CA $CERT_PATH/v2gRootCACert.pem -CAkey $KEY_PATH/v2gRootCA.key -passin pass:$password -set_serial 12346 -out $CERT_PATH/cpoSubCA1Cert.pem -days $VALIDITY_CPO_SUBCA1_CERT
...
openssl req -new -key $KEY_PATH/cpoSubCA2.key -passin pass:$password -config configs/$ISO_FOLDER/cpoSubCA2Cert.cnf -out $CSR_PATH/cpoSubCA2.csr
openssl x509 -req -in $CSR_PATH/cpoSubCA2.csr -extfile configs/$ISO_FOLDER/cpoSubCA2Cert.cnf -extensions ext -CA $CERT_PATH/cpoSubCA1Cert.pem -CAkey $KEY_PATH/cpoSubCA1.key -passin pass:$password -set_serial 12347 -days $VALIDITY_CPO_SUBCA2_CERT -out $CERT_PATH/cpoSubCA2Cert.pem
...
openssl req -new -key $KEY_PATH/seccLeaf.key -passin pass:$password -config configs/$ISO_FOLDER/seccLeafCert.cnf -out $CSR_PATH/seccLeafCert.csr
openssl x509 -req -in $CSR_PATH/seccLeafCert.csr -extfile configs/$ISO_FOLDER/seccLeafCert.cnf -extensions ext -CA $CERT_PATH/cpoSubCA2Cert.pem -CAkey $KEY_PATH/cpoSubCA2.key -passin pass:$password -set_serial 12348 -days $VALIDITY_SECC_LEAF_CERT -out $CERT_PATH/seccLeafCert.pem
...
openssl req -new -key $KEY_PATH/oemRootCA.key -passin pass:$password -config configs/$ISO_FOLDER/oemRootCACert.cnf -out $CSR_PATH/oemRootCA.csr
openssl x509 -req -in $CSR_PATH/oemRootCA.csr -extfile configs/$ISO_FOLDER/oemRootCACert.cnf -extensions ext -signkey $KEY_PATH/oemRootCA.key -passin pass:$password $SHA -set_serial 12349 -out $CERT_PATH/oemRootCACert.pem -days $VALIDITY_OEM_ROOT_CERT
...
openssl req -new -key $KEY_PATH/oemSubCA1.key -passin pass:$password -config configs/$ISO_FOLDER/oemSubCA1Cert.cnf -out $CSR_PATH/oemSubCA1.csr
openssl x509 -req -in $CSR_PATH/oemSubCA1.csr -extfile configs/$ISO_FOLDER/oemSubCA1Cert.cnf -extensions ext -CA $CERT_PATH/oemRootCACert.pem -CAkey $KEY_PATH/oemRootCA.key -passin pass:$password -set_serial 12350 -days $VALIDITY_OEM_SUBCA1_CERT -out $CERT_PATH/oemSubCA1Cert.pem
...
openssl req -new -key $KEY_PATH/oemSubCA2.key -passin pass:$password -config configs/$ISO_FOLDER/oemSubCA2Cert.cnf -out $CSR_PATH/oemSubCA2.csr
openssl x509 -req -in $CSR_PATH/oemSubCA2.csr -extfile configs/$ISO_FOLDER/oemSubCA2Cert.cnf -extensions ext -CA $CERT_PATH/oemSubCA1Cert.pem -CAkey $KEY_PATH/oemSubCA1.key -passin pass:$password -set_serial 12351 -days $VALIDITY_OEM_SUBCA2_CERT -out $CERT_PATH/oemSubCA2Cert.pem
...
openssl req -new -key $KEY_PATH/oemLeaf.key -passin pass:$password -config configs/$ISO_FOLDER/oemLeafCert.cnf -out $CSR_PATH/oemLeafCert.csr
openssl x509 -req -in $CSR_PATH/oemLeafCert.csr -extfile configs/$ISO_FOLDER/oemLeafCert.cnf -extensions ext -CA $CERT_PATH/oemSubCA2Cert.pem -CAkey $KEY_PATH/oemSubCA2.key -passin pass:$password -set_serial 12352 -days $VALIDITY_OEM_LEAF_CERT -out $CERT_PATH/oemLeafCert.pem
...
then
	...
	openssl req -new -key $KEY_PATH/moRootCA.key -passin pass:$password -config configs/$ISO_FOLDER/moRootCACert.cnf -out $CSR_PATH/moRootCA.csr
	openssl x509 -req -in $CSR_PATH/moRootCA.csr -extfile configs/$ISO_FOLDER/moRootCACert.cnf -extensions ext -signkey $KEY_PATH/moRootCA.key -passin pass:$password $SHA -set_serial 12353 -out $CERT_PATH/moRootCACert.pem -days $VALIDITY_MO_ROOT_CERT
	...
	openssl req -new -key $KEY_PATH/moSubCA1.key -passin pass:$password -config configs/$ISO_FOLDER/moSubCA1Cert.cnf -extensions ext -out $CSR_PATH/moSubCA1.csr
	openssl x509 -req -in $CSR_PATH/moSubCA1.csr -extfile configs/$ISO_FOLDER/moSubCA1Cert.cnf -extensions ext -CA $CERT_PATH/moRootCACert.pem -CAkey $KEY_PATH/moRootCA.key -passin pass:$password -set_serial 12354 -days $VALIDITY_MO_SUBCA1_CERT -out $CERT_PATH/moSubCA1Cert.pem
	...
	openssl req -new -key $KEY_PATH/moSubCA2.key -passin pass:$password -config configs/$ISO_FOLDER/moSubCA2Cert.cnf -out $CSR_PATH/moSubCA2.csr
	openssl x509 -req -in $CSR_PATH/moSubCA2.csr -extfile configs/$ISO_FOLDER/moSubCA2Cert.cnf -extensions ext -CA $CERT_PATH/moSubCA1Cert.pem -CAkey $KEY_PATH/moSubCA1.key -passin pass:$password -set_serial 12355 -days $VALIDITY_MO_SUBCA2_CERT -out $CERT_PATH/moSubCA2Cert.pem
	...
	openssl req -new -key $KEY_PATH/contractLeaf.key -passin pass:$password -config configs/$ISO_FOLDER/contractLeafCert.cnf -out $CSR_PATH/contractLeafCert.csr
	openssl x509 -req -in $CSR_PATH/contractLeafCert.csr -extfile configs/$ISO_FOLDER/contractLeafCert.cnf -extensions ext -CA $CERT_PATH/moSubCA2Cert.pem -CAkey $KEY_PATH/moSubCA2.key -passin pass:$password -set_serial 12356 -days $VALIDITY_CONTRACT_LEAF_CERT -out $CERT_PATH/contractLeafCert.pem
...
openssl req -new -key $KEY_PATH/cpsSubCA1.key -passin pass:$password -config configs/$ISO_FOLDER/cpsSubCA1Cert.cnf -out $CSR_PATH/cpsSubCA1.csr
openssl x509 -req -in $CSR_PATH/cpsSubCA1.csr -extfile configs/$ISO_FOLDER/cpsSubCA1Cert.cnf -extensions ext -CA $CERT_PATH/v2gRootCACert.pem -CAkey $KEY_PATH/v2gRootCA.key -passin pass:$password -set_serial 12357 -days $VALIDITY_CPS_SUBCA1_CERT -out $CERT_PATH/cpsSubCA1Cert.pem
...
openssl req -new -key $KEY_PATH/cpsSubCA2.key -passin pass:$password -config configs/$ISO_FOLDER/cpsSubCA2Cert.cnf -out $CSR_PATH/cpsSubCA2.csr
openssl x509 -req -in $CSR_PATH/cpsSubCA2.csr -extfile configs/$ISO_FOLDER/cpsSubCA2Cert.cnf -extensions ext -CA $CERT_PATH/cpsSubCA1Cert.pem -CAkey $KEY_PATH/cpsSubCA1.key -passin pass:$password -set_serial 12358 -days $VALIDITY_CPS_SUBCA2_CERT -out $CERT_PATH/cpsSubCA2Cert.pem
...
openssl req -new -key $KEY_PATH/cpsLeaf.key -passin pass:$password -config configs/$ISO_FOLDER/cpsLeafCert.cnf -out $CSR_PATH/cpsLeafCert.csr
openssl x509 -req -in $CSR_PATH/cpsLeafCert.csr -extfile configs/$ISO_FOLDER/cpsLeafCert.cnf -extensions ext -CA $CERT_PATH/cpsSubCA2Cert.pem -CAkey $KEY_PATH/cpsSubCA2.key -passin pass:$password -set_serial 12359 -days $VALIDITY_CPS_LEAF_CERT -out $CERT_PATH/cpsLeafCert.pem
