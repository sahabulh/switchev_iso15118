...
then
    ...
    openssl genpkey -algorithm $EC_CURVE $SYMMETRIC_CIPHER -pass pass:$password -out $KEY_PATH/cpsSubCA2.key
fi
...
scp $CSR_PATH/cpsSubCA2.csr root@10.1.2.102:$DEST$CSR_PATH
echo "CSR is sent to the CPO sub-CA 1."

# Create an X.509 certificate 
ssh root@10.1.2.102 "cd $DEST;openssl x509 -req -in $CSR_PATH/cpsSubCA2.csr -extfile configs/cpsSubCA2Cert.cnf -extensions ext -CA $CERT_PATH/cpsSubCA1Cert.pem -CAkey $KEY_PATH/cpsSubCA1.key -passin pass:$password -set_serial 12358 -out $CERT_PATH/cpsSubCA2Cert.pem -days $VALIDITY_CPS_SUBCA2_CERT"
echo "Certificate generation and signing is finished."
ssh root@10.1.2.102 "cd $DEST;rm $CSR_PATH/cpsSubCA2.csr"
echo "CSR is deleted from the CPO sub-CA 1."

ssh root@10.1.2.102 "cd $DEST;scp -o 'StrictHostKeyChecking no' $CERT_PATH/cpsSubCA2Cert.pem root@10.1.2.103:$DEST$CERT_PATH"
echo "Certificate is sent back to the CPO sub-CA 2."
ssh root@10.1.2.102 "cd $DEST;rm $CERT_PATH/cpsSubCA2Cert.pem"
