...
# Create an intermediate OEM sub-CA 1 certificate which is directly signed by the OEMRootCA certificate
...
then
    ...
    openssl genpkey -algorithm $EC_CURVE $SYMMETRIC_CIPHER -pass pass:$password -out $KEY_PATH/oemSubCA1.key
fi
...
scp $CSR_PATH/oemSubCA1.csr root@10.1.2.105:$DEST$CSR_PATH
echo "$HOST CSR is sent to the $MASTER."

# Create an X.509 certificate 
ssh root@10.1.2.105 "cd $DEST;openssl x509 -req -in $CSR_PATH/oemSubCA1.csr -extfile configs/oemSubCA1Cert.cnf -extensions ext -CA $CERT_PATH/oemRootCACert.pem -CAkey $KEY_PATH/oemRootCA.key -passin pass:$password -set_serial 12350 -out $CERT_PATH/oemSubCA1Cert.pem -days $VALIDITY_OEM_SUBCA1_CERT"
echo "$HOST Certificate generation and signing is finished."
ssh root@10.1.2.105 "cd $DEST;rm $CSR_PATH/oemSubCA1.csr"
echo "$HOST CSR is deleted from the $MASTER."

ssh root@10.1.2.105 "cd $DEST;scp -o 'StrictHostKeyChecking no' $CERT_PATH/oemSubCA1Cert.pem root@10.1.2.106:$DEST$CERT_PATH"
echo "$HOST Certificate is sent back to the $HOST."
ssh root@10.1.2.105 "cd $DEST;rm $CERT_PATH/oemSubCA1Cert.pem"
